...
public class EventbusBridgeTest extends WebTestBase {
  ...
  public void testReplyMessagesOutbound() throws Exception {
    ...
    client.websocket(websocketURI, ws -> {
      ...
      ws.handler(buff -> {
        ...
        // Now send back reply
        JsonObject reply = new JsonObject().put("type", "send").put("address", received.getString("replyAddress"))
                .put("body", "barfoo");
  ...
  public void testReplyToClientTimeout() throws Exception {
    ...
    client.websocket(websocketURI, ws -> {
      ...
      JsonObject msg = new JsonObject().put("type", "send").put("address", addr).put("replyAddress", replyAddress)
              .put("body", "foobar");
  ...
  public void testAwaitingReplyToClientTimeout() throws Exception {
    ...
    client.websocket(websocketURI, ws -> {
      ...
      ws.handler(buff -> {
        ...
        JsonObject rep_msg = new JsonObject().put("type", "send").put("address", received.getValue("replyAddress"))
                .put("replyAddress", secondReplyAddress).put("body", "three");
  ...
  public void testSendRequiresAuthorityNotLoggedIn() throws Exception {
    sockJSHandler.bridge(
            defaultOptions.addInboundPermitted(new PermittedOptions().setAddress(addr).setRequiredAuthority("admin")));
  ...
  private void testSend(String address, Object body, boolean headers) throws Exception {
    ...
    client.websocket(websocketURI, ws -> {
      ...
      System.out.println(msg.encode());//yezhang989 added.
